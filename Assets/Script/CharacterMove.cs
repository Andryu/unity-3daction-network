...
public class CharacterMove : MonoBehaviour {
  ...
  const float GRAVITY_POWER = 9.8f;
  const float STOPPING_DISTANCE = 0.6f;
  ...
  void Update () {
    if (characterController.isGrounded){
      ...
      if (arrived || distance < STOPPING_DISTANCE )
        arrived = true;
      ...
      if(!forceRotate) {
        ...
        transform.rotation = Quaternion.RotateTowards(transform.rotation, characterTargetRotation, rotationSpeed * Time.deltaTime);
         ...
         // gravity
    velocity += Vector3.down * GRAVITY_POWER * Time.deltaTime;
